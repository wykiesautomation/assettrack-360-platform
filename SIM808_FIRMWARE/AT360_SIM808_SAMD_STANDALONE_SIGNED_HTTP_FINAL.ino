...
static const char FW[] = "AT360-MADUINO-SIM808-V35-STANDALONE-12.0";
...
void loadConfig() {
  ...
  if (cfg.marker == LEGACY_CFG_MARKER) {
    cfg.marker = CFG_MARKER; cfg.autoUploadEnabled = (strlen(cfg.apn) && strlen(cfg.deviceUid) && strlen(cfg.deviceToken)) ? 1 : 0;
    safeCopy(cfg.apiHost,sizeof(cfg.apiHost),"sim808-ingest.wykiesautomation.co.za"); safeCopy(cfg.apiPath,sizeof(cfg.apiPath),"/api/v1/sim808-legacy-ingest"); memset(cfg.reserved,0,sizeof(cfg.reserved)); configStore.write(cfg);
  } else if (cfg.marker == PREVIOUS_CFG_MARKER) {
    cfg.marker=CFG_MARKER; safeCopy(cfg.apiHost,sizeof(cfg.apiHost),"sim808-ingest.wykiesautomation.co.za"); safeCopy(cfg.apiPath,sizeof(cfg.apiPath),"/api/v1/sim808-legacy-ingest"); configStore.write(cfg);
...
String jsonPayload() {
  ...
  String sequence = "gsm-" + (t.gpsTime.length() ? t.gpsTime : String(millis())) + "-" + String(millis());
  String body = "{\"device_id\":\"" + String(cfg.deviceUid) + "\",\"sequence\":\"" + sequence + "\",\"firmware\":\"" + String(FW) + "\",\"measurements\":[" +
...
bool sendGSM() {
  if (!strlen(cfg.deviceUid) || !strlen(cfg.deviceToken)) { emit("UPLOAD", "DEVICE_IDENTITY_NOT_SET"); return false; }
  if (!connectGPRS()) return false; modemCommand("AT+HTTPTERM",2000,false);
  if (!hasOK(modemCommand("AT+HTTPINIT",5000))) { emit("UPLOAD","HTTP_INIT_FAILED"); return false; }
  modemCommand("AT+HTTPPARA=\"CID\",1"); String body=jsonPayload(); String signature=at360Signature(String(cfg.deviceToken),body); emit("LEGACY_SIGNATURE",signature.length()==64?"READY":"FAILED");
  String url="http://"+String(cfg.apiHost)+String(cfg.apiPath); modemCommand(String("AT+HTTPPARA=\"URL\",\"")+url+"\""); modemCommand("AT+HTTPPARA=\"CONTENT\",\"application/json\""); modemCommand(String("AT+HTTPPARA=\"USERDATA\",\"X-AT360-Signature: ")+signature+"\"");
  String ready=modemCommand("AT+HTTPDATA="+String(body.length())+",15000",7000); if(ready.indexOf("DOWNLOAD")<0){emit("UPLOAD","HTTP_DATA_NOT_READY");modemCommand("AT+HTTPTERM",2000,false);return false;}
  MODEM.print(body);delay(1200);while(MODEM.available())MODEM.read();MODEM.println("AT+HTTPACTION=1");String result=waitForHttpAction(45000UL);emit("HTTP_ACTION",result.length()?result:"NO_RESPONSE");bool accepted=result.indexOf(",200,")>=0||result.indexOf(",201,")>=0||result.indexOf(",202,")>=0;emit("UPLOAD",accepted?"ACCEPTED":"FAILED");modemCommand("AT+HTTPTERM",3000,false);return accepted;
}
...
void setup() {
  ...
  emit("MODEM_POWER_POLICY", "MANUAL_POWER_KEY_ONLY_NO_AUTOMATIC_TOGGLE");
  emit("SAFE_BOOT", "MODEM_CHECK_DEFERRED_15_SECONDS");
...
void loop() {
  ...
  if (!simulationMode && !safeBootModemChecked && millis() - bootStartedMs >= SAFE_BOOT_MODEM_DELAY_MS) {
    ...
    if (modemResponding()) emit("MODEM", "READY_AFTER_SAFE_BOOT_DELAY");
    else emit("MODEM", "NOT_RESPONDING_POWER_KEY_NOT_TOUCHED");
  }
  ...
  if (autoUpload && safeBootModemChecked && millis() >= AUTO_UPLOAD_START_DELAY_MS) {
    ...
    bool due=!lastUploadAttemptMs && millis()-lastUploadMs>=intervalMs;
