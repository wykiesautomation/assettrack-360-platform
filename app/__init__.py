    ...
    with schema_startup_lock(app):
        for attempt in range(3):
            try:
                ...
                ensure_device_firmware_schema(app)
...
def ensure_device_firmware_schema(app):
    """Expand device firmware identity storage for full production build names."""
    if db.engine.dialect.name != "postgresql":
        return
    with db.engine.begin() as connection:
        connection.execute(text("ALTER TABLE device ALTER COLUMN firmware TYPE VARCHAR(100)"))
...
def create_app(test_config=None):
    ...
    from .legacy_sim808 import bp as legacy_sim808_bp
    app.register_blueprint(legacy_sim808_bp)
