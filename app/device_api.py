...
from . import db
from .models import Device,Asset,SignalDefinition,Reading,HardwareDeviceRegistration,MobileTrackerRegistration,DeviceChannelAssignment,DeviceCommand
...
def claim():
    ...
    # Device Centre historically stored hardware codes in MobileTrackerRegistration,
    # while Device Studio used HardwareDeviceRegistration. Accept both stores so the
    # current v2 endpoint is authoritative and the tester never needs a legacy fallback.
    reg=HardwareDeviceRegistration.query.filter_by(code_hash=h(code),provisioning_state='WAITING').first()
    registration_store='HARDWARE'
    if not reg:
        reg=MobileTrackerRegistration.query.filter_by(code_hash=h(code),used_at=None,provisioning_state='WAITING').first()
        registration_store='DEVICE_CENTRE'
        if reg and str(reg.onboarding_kind or '').upper()!='HARDWARE' and not reg.profile_code:
            return jsonify(error='hardware_claim_required'),422
    if not reg:return jsonify(error='invalid_claim_code'),404
    if now()>reg.expires_at.replace(tzinfo=reg.expires_at.tzinfo or timezone.utc):
    ...
    profile_code=str(profile.get('code') or '').upper()
    uid_prefixes={
    ...
    prefix=uid_prefixes.get(profile_code,'AT360-BOARD-')
    uid=prefix+board
    legacy_uids=('AT360-'+board,'AT360-BOARD-'+board)
    existing=Device.query.filter(Device.device_uid.in_((uid,)+legacy_uids)).order_by(Device.id).first()
    if existing and existing.customer_id!=reg.customer_id:return jsonify(error='board_already_claimed'),409
    if existing and existing.device_uid!=uid:
        canonical_conflict=Device.query.filter_by(device_uid=uid).first()
        if canonical_conflict and canonical_conflict.id!=existing.id:return jsonify(error='canonical_device_uid_conflict'),409
        existing.device_uid=uid
    ...
    dev.active=True;dev.api_token=token;dev.firmware=str(d.get('firmware',''))[:100];dev.capabilities=list(dict.fromkeys(profile.get('capabilities',[])+['PROFILE:'+profile['code']]))
    db.session.flush()
    if hasattr(reg,'claimed_device_id'):reg.claimed_device_id=dev.id
    reg.claimed_board_id=uid;reg.claimed_at=now();reg.provisioning_state='CLAIMED'
    if hasattr(reg,'used_at'):reg.used_at=now()
    ...
    db.session.commit();return jsonify(status='claimed',device_uid=uid,device_token=token,profile_code=profile['code'],registration_store=registration_store,io_studio=f'/devices/{dev.id}/io-studio'),201
...
def trends_limits(device_id):
    ...
    all_devices=Device.query.filter_by(customer_id=current_user.customer_id,active=True).order_by(Device.device_uid).all()
    ...
    return render_template('trends_limits.html',device=dev,all_devices=all_devices,signals=signals,signal=signal,index=index)
...
def telemetry():
    ...
    dev.last_seen=now();dev.firmware=str(data.get('firmware',dev.firmware or ''))[:100];db.session.commit();return jsonify(status='accepted',stored=stored,rejected=rejected,sequence=seq),202
