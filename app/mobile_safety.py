...
import os,secrets,time,requests,threading
...
_mobile_rate_lock=threading.Lock()
_mobile_rate_windows=defaultdict(deque)
def mobile_api_guard():
 """Bound mobile traffic without a database write on every API request.

 The previous guard inserted/locked/committed MobileRateBucket for every GPS,
 heartbeat and motion request. During Motion Safety bursts this contended with
 the actual telemetry transaction and could stall the single Gunicorn worker.
 """
 ...
 key=(dev.id,(request.endpoint or request.path)[:100]);current=time.monotonic()
 if request.path.endswith('/motion/samples'):limit=12
 elif request.path.endswith('/location/batch'):limit=120
 else:limit=90
 with _mobile_rate_lock:
  window=_mobile_rate_windows[key]
  while window and current-window[0]>=60:window.popleft()
  if len(window)>=limit:
   retry=max(1,int(60-(current-window[0])))
   return jsonify(error='rate_limit_exceeded',retry_after_seconds=retry),429
  window.append(current)
  if len(_mobile_rate_windows)>2000:
   stale=[k for k,v in _mobile_rate_windows.items() if not v or current-v[-1]>=120]
   for stale_key in stale[:1000]:_mobile_rate_windows.pop(stale_key,None)
 return None
def evidence(event):
...
def store_samples():
 device=device_auth()
 if not device:return jsonify(error='invalid_mobile_tracker_token'),401
 data=request.get_json(silent=True) or {};rows=data.get('samples') or []
 if not isinstance(rows,list) or not rows or len(rows)>100:return jsonify(error='invalid_sample_batch',minimum=1,maximum=100),400
 body_device=str(data.get('device_id') or '').strip().upper()
 if body_device and body_device!=device.device_uid.upper():return jsonify(error='device_identity_mismatch'),403
 prepared=[];sequences=[];rejected=[]
 for item in rows:
  if not isinstance(item,dict):rejected.append('invalid_item');continue
  seq=str(item.get('sequence') or '')[:140]
  if not seq:rejected.append('missing_sequence');continue
  try:
   sampled=datetime.fromisoformat(str(item.get('timestamp') or now().isoformat()).replace('Z','+00:00'))
   if not sampled.tzinfo:sampled=sampled.replace(tzinfo=timezone.utc)
   prepared.append((seq,sampled,max(0,min(100,float(item.get('dynamic_acceleration_ms2') or 0))),max(-180,min(180,float(item.get('roll_deg') or 0))),max(-180,min(180,float(item.get('pitch_deg') or 0))),max(0,min(300,float(item.get('speed_kmh') or 0))),max(0,min(5000,float(item.get('accuracy_m') or 0)))))
   sequences.append(seq)
  except (TypeError,ValueError,OverflowError):rejected.append(seq or 'invalid_value')
 existing={row[0] for row in MotionSafetySample.query.with_entities(MotionSafetySample.sequence).filter(MotionSafetySample.sequence.in_(sequences)).all()} if sequences else set()
 accepted=[];duplicates=[]
 try:
  for seq,sampled,dynamic,roll,pitch,speed,accuracy in prepared:
   if seq in existing:duplicates.append(seq);continue
   db.session.add(MotionSafetySample(customer_id=device.customer_id,asset_id=device.asset_id,device_id=device.id,sampled_at=sampled,dynamic_acceleration_ms2=dynamic,roll_deg=roll,pitch_deg=pitch,speed_kmh=speed,accuracy_m=accuracy,sequence=seq));accepted.append(seq);existing.add(seq)
  device.last_seen=now()
  if device.asset:device.asset.last_seen=device.last_seen
  db.session.commit()
 except Exception as exc:
  db.session.rollback()
  return jsonify(error='motion_store_temporarily_unavailable',retry_after_seconds=5,detail=type(exc).__name__),503
