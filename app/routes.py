...
def delete_device(device_id):
    """Delete one disabled device and its orphaned linked asset; retain the site."""
    ...
    if record.active:
        ...
        return redirect(request.referrer or url_for('main.devices'))
    ...
    if confirm_uid!=expected_uid or confirm_word!='DELETE':
        flash(f'Type the exact Device UID {expected_uid} and DELETE.','error')
        return redirect(request.referrer or url_for('main.devices'))
    ...
    try:
        ...
        def remove_fk_children(parent_table,parent_id,parent_customer_id):
            for table_name in inspector.get_table_names():
                ...
                for fk in inspector.get_foreign_keys(table_name):
                    ...
                    # Device-owned rows must disappear. For asset deletion all
                    # asset-owned history/config rows must disappear as well.
                    if parent_table=='device' and columns.get(column_name,{}).get('nullable',True):
                        db.session.execute(text(
                            f'UPDATE {table_sql} SET {column_sql}=NULL WHERE {column_sql}=:parent_id'
                        ),{'parent_id':parent_id})
                    else:
                        db.session.execute(text(
                            f'DELETE FROM {table_sql} WHERE {column_sql}=:parent_id'
                        ),{'parent_id':parent_id})

        remove_fk_children('device',record.id,customer_id)
        db.session.execute(text('DELETE FROM device WHERE id=:device_id AND customer_id=:customer_id'),{
            'device_id':record.id,'customer_id':customer_id,
        })
        db.session.flush()

        # A device-created/linked asset must not remain as a ghost card. Delete
        # it only when no other device is linked to it. The parent Site remains.
        asset_deleted=False
        if asset_id:
            other_devices=db.session.execute(text(
                'SELECT COUNT(*) FROM device WHERE asset_id=:asset_id AND customer_id=:customer_id'
            ),{'asset_id':asset_id,'customer_id':customer_id}).scalar_one()
            if not other_devices:
                remove_fk_children('asset',asset_id,customer_id)
                db.session.execute(text(
                    'DELETE FROM asset WHERE id=:asset_id AND customer_id=:customer_id'
                ),{'asset_id':asset_id,'customer_id':customer_id})
                asset_deleted=True

        device_remaining=db.session.execute(text(
            'SELECT COUNT(*) FROM device WHERE id=:device_id AND customer_id=:customer_id'
        ),{'device_id':device_id,'customer_id':customer_id}).scalar_one()
        if device_remaining:
            raise RuntimeError('device_delete_verification_failed')
        if asset_deleted:
            asset_remaining=db.session.execute(text(
                'SELECT COUNT(*) FROM asset WHERE id=:asset_id AND customer_id=:customer_id'
            ),{'asset_id':asset_id,'customer_id':customer_id}).scalar_one()
            if asset_remaining:
        ...
        db.session.commit()
        if asset_deleted:
            flash(f'Device {expected_uid} and asset {asset_name} permanently deleted. Site retained.','ok')
        else:
