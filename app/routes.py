...
from flask import current_app, Blueprint, render_template, request, redirect, url_for, flash, jsonify, abort, session, current_app, send_from_directory, send_file
...
MOBILE_TRACKER_DEVICE_TYPES=('MOBILE_WEB_TRACKER','ANDROID_MOBILE_TRACKER','IOS_MOBILE_TRACKER','MOBILE_TRACKER')
def mobile_tracker_device():
    token=request.headers.get('Authorization','').removeprefix('Bearer ').strip()
    if not token:return None
    return Device.query.filter(Device.api_token==token,Device.active.is_(True),Device.device_type.in_(MOBILE_TRACKER_DEVICE_TYPES)).first()
...
def device_connectivity_state(device, reference_time=None):
    """Honest push-device connectivity based on age of last accepted contact.

    A telemetry POST proves contact only at that instant. It cannot prove that a
    Wi-Fi board remains connected after the request. ESP32 hardware therefore
    shows ONLINE only during a short 20-second live-contact window, then
    RECENT_CONTACT until 90 seconds, DELAYED until five minutes, and OFFLINE
    afterwards. This prevents an unplugged board being labelled ONLINE merely
    because its last telemetry is still inside the old 90-second window.
    """
    if not device or not device.last_seen:
        return 'NEVER_SEEN'
    current=reference_time or utcnow()
    age_seconds=max(0,(current-aware(device.last_seen)).total_seconds())
    device_type=str(device.device_type or '').upper()
    push_hardware=device_type in ('ESP32_WROOM32_REMOTE_IO','ESP32D_38PIN_REMOTE_IO','ESP32_REMOTE_IO')
    online_seconds=20 if push_hardware else 90
    if age_seconds<=online_seconds:
        return 'ONLINE'
    if push_hardware and age_seconds<=90:
...
def asset_status(asset):
    ...
    connectivity=device_connectivity_state(device)
    if connectivity=='NEVER_SEEN':
        return 'UNASSIGNED' if not device else 'NEVER_SEEN'
    ...
    return open_alarm.severity if open_alarm else 'ONLINE'
...
def enforce_passthrough_signal(sig,key):
    ...
    if key=='airtime_balance_zar':sig.warning_low=20;sig.critical_low=10
...
def _queue_balance_notification(sig,severity,state,value,alarm=None):
    # One notification per state transition, with a hard 24-hour reminder cooldown.
    profile=WorkspaceProfile.query.filter_by(customer_id=sig.customer_id).first()
    recipient=((profile.contact_email if profile else None) or (profile.billing_email if profile else None) or '').strip()
    if not recipient:return
    asset=db.session.get(Asset,sig.asset_id)
    subject=f'{severity}: {sig.label} {state} - {asset.name if asset else "AssetTrack 360 device"}'
    cutoff=utcnow()-timedelta(hours=24)
    recent=EmailNotificationLog.query.filter_by(customer_id=sig.customer_id,asset_id=sig.asset_id,subject=subject).filter(EmailNotificationLog.created_at>=cutoff).first()
    if recent:return
    row=EmailNotificationLog(customer_id=sig.customer_id,asset_id=sig.asset_id,alarm_id=alarm.id if alarm and alarm.id else None,recipient=recipient,subject=subject,severity=severity,state='QUEUED')
    db.session.add(row)
    db.session.add(SecurityAuditEvent(customer_id=sig.customer_id,asset_id=sig.asset_id,event_type='SIM_BALANCE_NOTIFICATION_QUEUED',actor_type='SYSTEM',safe_summary=f'{sig.key} {state} at {value:.2f}; 24-hour cooldown applied'))

def evaluate_alarm(sig,value):
    severity=None; msg=None; state=None
    if sig.key=='airtime_balance_zar':
        if value<10:severity='CRITICAL';state='CRITICAL';msg=f'Airtime critically low: R{value:.2f}. Reload the SIM.'
        elif value<20:severity='WARNING';state='LOW';msg=f'Airtime low: R{value:.2f}. Reload the SIM soon.'
    elif sig.key=='data_remaining_mb':
        if value<100:severity='CRITICAL';state='CRITICAL';msg=f'Mobile data critically low: {value:.0f} MB remaining.'
        elif value<250:severity='WARNING';state='LOW';msg=f'Mobile data low: {value:.0f} MB remaining.'
    else:
    ...
    previous=existing.severity if existing else None
    if severity and not existing:
        existing=Alarm(customer_id=sig.customer_id,asset_id=sig.asset_id,signal_id=sig.id,severity=severity,message=msg,value=value);db.session.add(existing);db.session.flush()
        if state:_queue_balance_notification(sig,severity,state,value,existing)
    elif severity and existing:
        existing.severity=severity;existing.message=msg;existing.value=value
        if state and previous!=severity:_queue_balance_notification(sig,severity,state,value,existing)
    elif existing:
        # Airtime critical recovery uses R15 hysteresis. LOW clears only at R20.
        if sig.key=='airtime_balance_zar' and existing.severity=='CRITICAL' and value<15:return
        existing.state='CLOSED';existing.note=(existing.note or '')+' Auto-closed after return to normal.'
        if sig.key in ('airtime_balance_zar','data_remaining_mb'):_queue_balance_notification(sig,'INFO','RECOVERED',value,existing)
...
def dashboard():
    ...
    for asset in assets:
        ...
        device_fresh=bool(device and device_connectivity_state(device,now)=='ONLINE')
    ...
    online=sum(1 for d in devices if device_connectivity_state(d,now)=='ONLINE')
...
def test_data_cleanup():
    ...
    for asset in assets:
        device_count=Device.query.filter_by(customer_id=customer_id,asset_id=asset.id).count()
        items.append({'asset':asset,'active_device':active_device_for(asset),'devices':device_count,'orphaned_device_record':device_count==0 and str(asset.asset_type or '').upper()=='TRACKER','locations':Location.query.filter_by(customer_id=customer_id,asset_id=asset.id).count(),'readings':Reading.query.filter_by(customer_id=customer_id,asset_id=asset.id).count(),'alarms':Alarm.query.filter_by(customer_id=customer_id,asset_id=asset.id).count()})
    sites=Site.query.filter_by(customer_id=customer_id).order_by(Site.name,Site.id).all()
...
def delete_test_asset():
    ...
    try:
        ...
        device_count=Device.query.filter_by(customer_id=customer_id,asset_id=asset.id).count();was_orphaned_tracker=device_count==0 and str(asset.asset_type or '').upper()=='TRACKER';name=asset.name;db.session.delete(asset);db.session.commit();message=(f'Deleted-device tracker residue {name} and all retained history deleted. Site retained.' if was_orphaned_tracker else f'Asset {name} and retained history deleted. Site retained.');return jsonify(ok=True,message=message)
...
def analyse_tracking_points(rows):
    ...
    # Build a customer-facing measured trace from the same validated observations.
    # This preserves a real drive across intermittent mobile uploads while refusing
    # stale gaps, impossible speeds and large coordinate jumps. It never road-snaps.
    display_segments=[];display_current=[]
    for point in valid:
        if not display_current:
            display_current=[point];continue
        previous=display_current[-1]
        seconds=(datetime.fromisoformat(point['timestamp'])-datetime.fromisoformat(previous['timestamp'])).total_seconds()
        metres=_distance_dict(previous,point)*1000
        implied_kmh=(metres/seconds)*3.6 if seconds>0 else 9999
        # Android background delivery can be delayed; sampled GPS continuity remains
        # valid for up to ten minutes when the implied movement is physically plausible.
        continuous=0<seconds<=600 and metres<=5000 and implied_kmh<=180
        if continuous:
            display_current.append(point)
        else:
            if len(display_current)>=2:display_segments.append(display_current)
            display_current=[point]
    if len(display_current)>=2:display_segments.append(display_current)

    # The customer-facing KPIs must be calculated from the exact same measured
    # segments drawn as the blue route. Previously the map used display_segments
    # while distance, duration and maximum speed still used the stricter analytical
    # segments, producing a long blue route with only 0.62 km and 3 minutes.
    display_distance_km=0.0;display_moving_seconds=0.0;display_maximum_speed=0.0;display_journeys=[]
    for number,segment in enumerate(display_segments,1):
        segment_distance_km=0.0;segment_seconds=0.0;segment_maximum=0.0
        for a,b in zip(segment,segment[1:]):
            seconds=(datetime.fromisoformat(b['timestamp'])-datetime.fromisoformat(a['timestamp'])).total_seconds()
            if seconds<=0 or seconds>600:continue
            km=_distance_dict(a,b)
            implied_kmh=km/(seconds/3600.0)
            if km*1000>5000 or implied_kmh>180:continue
            jitter=max(6.0,min(25.0,(float(a.get('accuracy') or 0)+float(b.get('accuracy') or 0))*0.35))
            if km*1000<=jitter:continue
            segment_distance_km+=km;segment_seconds+=seconds
            reported=max(float(a.get('speed') or 0),float(b.get('speed') or 0))
            # Calculated speed recovers phone samples where Android reported zero or
            # an understated value, while the same plausibility gate blocks spikes.
            segment_maximum=max(segment_maximum,reported,implied_kmh)
        if segment_distance_km>0:
            display_distance_km+=segment_distance_km;display_moving_seconds+=segment_seconds
            display_maximum_speed=max(display_maximum_speed,segment_maximum)
            display_journeys.append({'number':number,'started_at':segment[0]['timestamp'],'ended_at':segment[-1]['timestamp'],'point_count':len(segment),'distance_km':round(segment_distance_km,3),'maximum_speed':round(segment_maximum,1),'movement_minutes':round(segment_seconds/60)})
    current_speed=float(valid[-1].get('speed') or 0) if valid else 0.0
    return {'points':accepted,'last':last,'accepted':accepted,'rejected':rejected,'segments':segments,'display_segments':display_segments,'journeys':display_journeys,'stops':stops,'total_km':round(display_distance_km,2),'distance_km':round(display_distance_km,2),'current_speed':round(current_speed,1),'max_speed':round(display_maximum_speed),'maximum_speed':round(display_maximum_speed),'moving_minutes':round(display_moving_seconds/60),'movement_minutes':round(display_moving_seconds/60),'stopped_minutes':stopped_minutes,'stationary_minutes':stopped_minutes,'rejection_counts':rejection_counts,'confidence':confidence,'state':state,'raw_count':len(ordered),'movement_count':sum(len(x) for x in display_segments),'drift_count':max(0,len(valid)-sum(len(x) for x in display_segments)),'rejected_count':len(rejected),'low_quality_count':sum(1 for x in valid if x['accuracy']>50),'raw_speed':valid[-1]['speed'] if valid else None,'observations':valid}
...
def tracking_history(asset_id):
    ...
    motion_rows=Live360SafetyEvent.query.filter(Live360SafetyEvent.customer_id==tenant_id(),Live360SafetyEvent.asset_id==asset.id,Live360SafetyEvent.sampled_at>=start,Live360SafetyEvent.sampled_at<=end,Live360SafetyEvent.status.in_(['RECORDED','CONFIRMED'])).order_by(Live360SafetyEvent.sampled_at).limit(500).all()
    ...
    episodes=[];last_by_type={}
    for event in events:
        kind=event.get('type');stamp=event.get('timestamp');parsed_stamp=None
        try:parsed_stamp=datetime.fromisoformat(str(stamp).replace(' UTC','+00:00').replace('Z','+00:00'))
        except (TypeError,ValueError):pass
        previous=last_by_type.get(kind)
        if previous and parsed_stamp and previous.get('_stamp') and abs((parsed_stamp-previous['_stamp']).total_seconds())<600:
            previous['count']=int(previous.get('count') or 1)+1
            previous['message']=event.get('message') or previous.get('message')
            previous['timestamp']=event.get('timestamp') or previous.get('timestamp')
            continue
        item=dict(event);item['count']=1;item['_stamp']=parsed_stamp;episodes.append(item);last_by_type[kind]=item
    for event in episodes:event.pop('_stamp',None)
...
def tracking_safety_save(asset_id):
    ...
    try:
        speed=max(1.0,min(60.0,float(request.form.get('speed_limit_kmh') or 60)))
        submitted=request.form.get('zones_json')
        if submitted is None:raise ValueError('zones_json missing')
        zones=json.loads(submitted or '[]')
        if not isinstance(zones,list):raise ValueError('zones must be a list')
        # Server-side fallback for browsers that fail to execute the zone-builder JavaScript.
        # A submitted draft is added only when zones_json is empty or the named draft is not already present.
        draft_lat=request.form.get('draft_zone_latitude','').strip();draft_lon=request.form.get('draft_zone_longitude','').strip()
        draft_name=request.form.get('draft_zone_name','').strip()
        if draft_lat and draft_lon and (draft_name or not zones):
            candidate_name=draft_name or f'Zone {len(zones)+1}'
            if not any(str(item.get('name') or '').strip().casefold()==candidate_name.casefold() for item in zones if isinstance(item,dict)):
                zones.append({'name':candidate_name,'latitude':draft_lat,'longitude':draft_lon,'radius_m':request.form.get('draft_zone_radius_m') or 500,'rule':request.form.get('draft_zone_rule') or 'KEEP_IN'})
        clean=[];names=set()
        for zone in zones[:20]:
            if not isinstance(zone,dict):raise ValueError('zone must be an object')
            name=str(zone.get('name') or '').strip()[:80]
            if not name:raise ValueError('zone name required')
            folded=name.casefold()
            if folded in names:raise ValueError('duplicate zone name')
            names.add(folded)
            latitude=float(zone['latitude']);longitude=float(zone['longitude']);radius=float(zone.get('radius_m') or 250)
            if not (-90<=latitude<=90) or not (-180<=longitude<=180):raise ValueError('invalid coordinates')
            clean.append({'name':name,'latitude':latitude,'longitude':longitude,'radius_m':max(50.0,min(50000.0,radius)),'rule':'KEEP_OUT' if str(zone.get('rule') or '').upper()=='KEEP_OUT' else 'KEEP_IN'})
        ...
        previous=dict(meta.get('tracking_safety') or {})
        previous.update({'speed_limit_kmh':speed,'zones':clean,'rules':rules,'updated_at':utcnow().isoformat(),'updated_by':current_user.id})
        meta['tracking_safety']=previous
        asset.metadata_json=meta
        db.session.add(asset);db.session.commit()
    except (ValueError,TypeError,KeyError,json.JSONDecodeError):
        db.session.rollback();flash('Geofence Zones were not saved because the submitted zone data is invalid. Check the name, centre point and radius.','error');return redirect(url_for('main.tracking_history',asset_id=asset.id))
    except Exception:
        db.session.rollback();current_app.logger.exception('Geofence zone save failed for asset %s',asset.id);flash('Geofence Zones could not be saved. No partial change was committed.','error');return redirect(url_for('main.tracking_history',asset_id=asset.id))
    flash(f'Geofence Zones saved successfully. {len(clean)} zone(s) stored.','ok')
    return redirect(url_for('main.tracking_history',asset_id=asset.id,device_id=request.form.get('device_id',type=int)))
...
def asset_view(asset_id):
    ...
    # Shared asset connectivity truth for header, process cards and output safety.
    contact_seconds=None
    contact_minutes=None
    if device and device.last_seen:
        contact_seconds=max(0,int((now-aware(device.last_seen)).total_seconds()))
        contact_minutes=contact_seconds//60
    connectivity_state=device_connectivity_state(device,now)
    if contact_seconds is None:
        connectivity_detail='No contact received'
    elif contact_seconds<60:
        connectivity_detail=f'Last contact {contact_seconds} sec ago'
    else:
        connectivity_detail=f'Last contact {contact_minutes} min ago'
    asset_connectivity={
    ...
    # The header must describe connectivity, never reuse HEALTHY as a connection state.
    asset.status=asset_status(asset)
...
def create_output_command(asset_id):
 asset=Asset.query.filter_by(id=asset_id,customer_id=tenant_id()).first_or_404();device=active_device_for(asset);profile=profile_for_device(device)
 if not device or not profile:abort(404)
 if device_connectivity_state(device,utcnow())!='ONLINE':
...
def hardware_device_claim():
    ...
    uid_prefixes={
        'AT360_ESP32_WROOM32':'AT360-WROOM32-',
        'AT360_ESP32D_EXPANDED':'AT360-ESP32D-',
        'AT360_SIM808_TRACKER_2AI_2DO':'AT360-SIM808-',
        'AT360_LILYGO_T_SIM7000G':'AT360-TSIM7000G-',
    }
    prefix=uid_prefixes.get(str(expected_profile.get('code') or '').upper(),'AT360-BOARD-');final_uid=prefix+board_id
    legacy_uids=('AT360-'+board_id,'AT360-BOARD-'+board_id)
    existing=Device.query.filter(Device.device_uid.in_((final_uid,)+legacy_uids)).order_by(Device.id).first()
    ...
    if existing and existing.device_uid!=final_uid:
        canonical_conflict=Device.query.filter_by(device_uid=final_uid).first()
        if canonical_conflict and canonical_conflict.id!=existing.id:return jsonify(error='canonical_device_uid_conflict'),409
        existing.device_uid=final_uid
...
@bp.post('/api/v1/android/register')
def mobile_tracker_register():
    ...
    if str(reg.onboarding_kind or '').upper()=='HARDWARE' or reg.profile_code:
        return jsonify(error='mobile_registration_code_required'),422
    ...
    client_version=str(data.get('client_version') or 'mobile-web-2.0')[:40];platform=re.sub(r'[^A-Z0-9_-]','',str(data.get('platform') or 'WEB').upper())[:20] or 'WEB';is_web_client=client_version.lower().startswith('mobile-web');device_type='MOBILE_WEB_TRACKER' if is_web_client else 'ANDROID_MOBILE_TRACKER' if platform=='ANDROID' else 'IOS_MOBILE_TRACKER' if platform=='IOS' else 'MOBILE_TRACKER';device=Device(customer_id=reg.customer_id,asset_id=reg.asset_id,device_uid=reg.device_uid,device_type=device_type,api_token=token,active=True,firmware=client_version,capabilities=['GPS','PHONE_BATTERY','USER_CONSENT_REQUIRED','PLATFORM:'+platform,'PROFILE:AT360_MOBILE_TRACKER'])
...
def mobile_tracker_location_batch():
    ...
    # The authenticated batch itself is the authoritative connectivity heartbeat.
    # Use server receipt time even when the phone replays old/duplicate queued points.
    device.last_seen=received_at
    device.asset.last_seen=received_at
    device.asset.status='ONLINE'
    batch_versions=[str(p.get('client_version') or '')[:40] for p in points if isinstance(p,dict) and p.get('client_version')]
    device.firmware=(batch_versions[-1] if batch_versions else device.firmware)
    try:
        db.session.commit()
    except Exception:
        db.session.rollback()
        current_app.logger.exception('Mobile batch commit failed device_uid=%s accepted=%s duplicates=%s rejected=%s',device.device_uid,len(accepted),len(duplicates),len(rejected))
        return jsonify(error='mobile_batch_commit_failed',device_uid=device.device_uid),500
    latest=Location.query.filter_by(customer_id=device.customer_id,asset_id=device.asset_id).order_by(desc(Location.sampled_at)).first()
    response=jsonify(status='processed',stored=True,last_seen_updated=True,device_uid=device.device_uid,asset_id=device.asset_id,accepted=accepted,duplicates=duplicates,rejected=rejected,accepted_count=len(accepted),duplicate_count=len(duplicates),rejected_count=len(rejected),last_seen=received_at.isoformat(),latest_location_sequence=latest.sequence if latest else None,latest_location_at=aware(latest.sampled_at).isoformat() if latest and latest.sampled_at else None,server_time=utcnow().isoformat())
    response.headers['Cache-Control']='no-store, no-cache, must-revalidate, max-age=0'
    current_app.logger.info('Mobile batch stored device_uid=%s asset_id=%s accepted=%s duplicates=%s rejected=%s last_seen=%s',device.device_uid,device.asset_id,len(accepted),len(duplicates),len(rejected),received_at.isoformat())
    return response,207 if rejected else 202
...
def mobile_motion_capabilities():
    ...
    consent=consent_for_device(device)
    if not consent or not consent.active:return jsonify(error='consent_inactive'),403
    prefixes=('MOTION_API:','ORIENTATION_API:','MOTION_PERMISSION:','MOTION_PROFILE:','CAPABILITY_SOURCE:','CAPABILITY_CLIENT:')
    if request.method=='POST':
        data=request.get_json(silent=True) or {}
        if str(data.get('device_id','')).upper()!=device.device_uid.upper():return jsonify(error='device_identity_mismatch'),403
        ...
        capabilities.extend([f'MOTION_API:{str(motion).lower()}',f'ORIENTATION_API:{str(orientation).lower()}',f'MOTION_PERMISSION:{permission}','MOTION_PROFILE:2.0',f"CAPABILITY_SOURCE:{'ANDROID_NATIVE' if data.get('native_app') else 'WEB'}",f"CAPABILITY_CLIENT:{str(data.get('client_version') or 'unknown')[:40]}"] )
...
def mobile_event():
    ...
    if event=='TRACKING_STARTED':
        ...
        # Production noise gate: weak phone candidates are acknowledged but not stored.
        duration=float(data.get('orientation_duration_seconds') or 0)
        speed_delta=abs(float(before or 0)-float(after or 0))
        if event in ('HARSH_BRAKING','SEVERE_BRAKING','HARSH_ACCELERATION') and (float(confidence or 0)<0.80 or max(float(before or 0),float(after or 0))<10 or speed_delta<8):
            return jsonify(status='filtered',reason='motion_evidence_below_threshold'),202
        if event=='ABNORMAL_TILT' and (float(confidence or 0)<0.90 or duration<10):
            return jsonify(status='filtered',reason='tilt_not_sustained'),202
        cooldown_seconds=900 if event=='ABNORMAL_TILT' else 300 if event in ('HARSH_BRAKING','SEVERE_BRAKING','HARSH_ACCELERATION') else 60
        prior=Live360SafetyEvent.query.filter(Live360SafetyEvent.device_id==device.id,Live360SafetyEvent.event_type==event,Live360SafetyEvent.sampled_at>=sampled-timedelta(seconds=cooldown_seconds)).order_by(desc(Live360SafetyEvent.sampled_at)).first()
        if prior:return jsonify(status='suppressed',reason='event_cooldown',existing_event_id=prior.id),202
...
def delete_device(device_id):
    """Permanently remove a disabled device and its orphaned linked asset.

    The parent site and all unrelated devices/assets are always retained.
    """
    ...
    site_id=record.asset.site_id if record.asset else None

    try:
        inspector=inspect(db.engine)
        preparer=db.engine.dialect.identifier_preparer

        def remove_foreign_key_references(parent_table,parent_id):
            """Delete mandatory children and detach nullable historical references."""
            for table_name in inspector.get_table_names():
                ...
                for fk in inspector.get_foreign_keys(table_name):
                    ...
                    nullable=columns.get(column_name,{}).get('nullable',True)
                    # Device deletion detaches nullable claim/audit references.
                    # Orphan asset deletion removes every asset-owned row so no
                    # ghost card, telemetry, map point or selector entry remains.
                    if parent_table=='device' and nullable:
                        db.session.execute(
                            text(f'UPDATE {table_sql} SET {column_sql}=NULL WHERE {column_sql}=:parent_id'),
                            {'parent_id':parent_id},
                        )
                    else:
                        db.session.execute(
                            text(f'DELETE FROM {table_sql} WHERE {column_sql}=:parent_id'),
                            {'parent_id':parent_id},
                        )

        # Revoke first, remove all device-owned rows, then delete the identity.
        record.active=False
        record.api_token='REVOKED-'+secrets.token_urlsafe(32)
        db.session.flush()
        remove_foreign_key_references('device',record.id)
        db.session.execute(
            text('DELETE FROM device WHERE id=:device_id AND customer_id=:customer_id'),
            {'device_id':record.id,'customer_id':customer_id},
        )
        db.session.flush()

        # Delete only an orphaned linked asset. A shared asset is retained if
        # another device remains linked. The parent Site is never deleted here.
        asset_deleted=False
        if asset_id:
            linked_count=db.session.execute(
                text('SELECT COUNT(*) FROM device WHERE asset_id=:asset_id AND customer_id=:customer_id'),
                {'asset_id':asset_id,'customer_id':customer_id},
            ).scalar_one()
            if int(linked_count or 0)==0:
                remove_foreign_key_references('asset',asset_id)
                db.session.execute(
                    text('DELETE FROM asset WHERE id=:asset_id AND customer_id=:customer_id'),
                    {'asset_id':asset_id,'customer_id':customer_id},
                )
                asset_deleted=True

        # Transaction guards prevent partial or over-broad deletion.
        if db.session.execute(
            text('SELECT COUNT(*) FROM device WHERE id=:device_id AND customer_id=:customer_id'),
            {'device_id':device_id,'customer_id':customer_id},
        ).scalar_one():
            raise RuntimeError('device_delete_verification_failed')
        if asset_deleted and db.session.execute(
            text('SELECT COUNT(*) FROM asset WHERE id=:asset_id AND customer_id=:customer_id'),
            {'asset_id':asset_id,'customer_id':customer_id},
        ).scalar_one():
            raise RuntimeError('asset_delete_verification_failed')
        if site_id and not db.session.get(Site,site_id):
            raise RuntimeError('site_retention_guard_failed')

        db.session.commit()
        if asset_deleted:
            flash(f'Device {expected_uid} and orphaned asset {asset_name} permanently deleted. Site retained.','ok')
        else:
            flash(f'Device {expected_uid} permanently deleted. Shared asset and site retained.','ok')
    except Exception as exc:
        db.session.rollback()
        current_app.logger.exception('Permanent device delete failed device_id=%s customer_id=%s',device_id,customer_id)
        flash(f'Deletion failed safely: {type(exc).__name__}. No partial deletion was committed.','error')
...
def ingest():
    ...
    if loc.get('latitude') is not None and loc.get('longitude') is not None:
        try:
            ...
            if accuracy>100:raise ValueError('gps_accuracy_too_poor')
    ...
    device.last_seen=utcnow();asset.last_seen=utcnow();device.firmware=str(payload.get('firmware') or device.firmware or '')[:100]
...
def analyse_safety_twin_points(rows):
    ...
    for row in rows:
        ...
        # GPS drift at walking/noise speed must never create a route or MOVING state.
        # SIM808 stationary fixes below 5 km/h are retained as stationary evidence only.
        if item['speed'] < 5.0:
            item['speed']=0.0;candidate=[];drift.append(item)
        ...
        envelope=max(20.0,min(100.0,previous_acc+acc))
...
def safety_twin(asset_id):
    asset=Asset.query.filter_by(id=asset_id,customer_id=tenant_id()).first_or_404()
    now=utcnow();all_rows=Location.query.filter_by(customer_id=tenant_id(),asset_id=asset.id).order_by(desc(Location.sampled_at)).limit(250).all();all_rows=list(reversed(all_rows))
    # Live cards represent only the current recent session, never an old movement trace.
    session_cutoff=now-timedelta(minutes=30)
    rows=[row for row in all_rows if aware(row.sampled_at)>=session_cutoff]
    twin=analyse_safety_twin_points(rows)
    latest=rows[-1] if rows else (all_rows[-1] if all_rows else None)
    recent_stationary=rows[-3:] if len(rows)>=3 else []
    if recent_stationary and all(float(row.speed_kmh or 0)<5.0 for row in recent_stationary):
        twin['state']='STATIONARY';twin['distance_km']=0.0;twin['movement_minutes']=0;twin['maximum_speed']=0.0;twin['movement']=[];twin['movement_count']=0
    ...
    device_type=str(device.device_type or '').upper() if device else ''
    is_phone=device_type in {'ANDROID_MOBILE_TRACKER','MOBILE_TRACKER','IOS_MOBILE_TRACKER','MOBILE_WEB_TRACKER'}
    native_android=is_phone and 'CAPABILITY_SOURCE:ANDROID_NATIVE' in caps
    is_sim808='SIM808' in device_type or any('SIM808' in x for x in caps)
    safety_cards=[]
    if latest:
        safety_cards.append({'title':'GPS Truth','detail':'Validated position, speed and GPS uncertainty.','status':'ACTIVE','ready':True,'setup_url':None})
    else:
        safety_cards.append({'title':'GPS Truth','detail':'Waiting for the first accepted GPS observation.','status':'WAITING FOR GPS','ready':False,'setup_url':None})
    if zones:
        safety_cards.append({'title':'Geofence Monitoring','detail':f"{len(zones)} saved Keep In/Keep Out zone(s).",'status':'CONFIGURED','ready':True,'setup_url':url_for('main.tracking_history',asset_id=asset.id)+'#rules'})
    else:
        safety_cards.append({'title':'Geofence Monitoring','detail':'Create a zone from the latest accepted position.','status':'NOT CONFIGURED','ready':False,'setup_url':url_for('main.tracking_history',asset_id=asset.id)+'#rules'})
    safety_cards.append({'title':'Unexpected Movement','detail':'Uses validated GPS displacement outside the uncertainty envelope.','status':'AVAILABLE' if latest else 'WAITING FOR GPS','ready':bool(latest),'setup_url':url_for('main.tracking_history',asset_id=asset.id)+'#rules'})
    if native_android and motion_sensor:
        safety_cards.append({'title':'Possible Impact','detail':'Uses native phone motion plus GPS evidence.','status':'AVAILABLE' if motion_enabled and mounted else 'SETUP REQUIRED','ready':bool(motion_enabled and mounted),'setup_url':url_for('motion_safety.setup',asset_id=asset.id)})
        if orientation_sensor:
            safety_cards.append({'title':'Abnormal Tilt','detail':'Uses the phone orientation sensor and fixed-mount baseline.','status':'AVAILABLE' if motion_enabled and mounted else 'SETUP REQUIRED','ready':bool(motion_enabled and mounted),'setup_url':url_for('motion_safety.setup',asset_id=asset.id)})
        if vehicle_profile:
            safety_cards.append({'title':'Harsh Driving','detail':'Cross-validates native motion against GPS speed change.','status':'AVAILABLE' if motion_enabled and mounted else 'SETUP REQUIRED','ready':bool(motion_enabled and mounted),'setup_url':url_for('motion_safety.setup',asset_id=asset.id)})
    if is_sim808:
        safety_cards.append({'title':'Tracker Health','detail':'SIM808 battery, GSM contact and tracker connectivity.','status':connectivity.replace('_',' '),'ready':connectivity=='ONLINE','setup_url':None})
    device_safety_mode='ANDROID MOTION SAFETY' if native_android else 'SIM808 GPS SAFETY' if is_sim808 else 'GPS SAFETY'
