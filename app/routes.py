...
def analyse_tracking_points(rows):
    """Build an evidence-safe mobile route and real stop episodes.

    Phone GPS may report zero speed while walking or driving slowly, so movement is
    confirmed from accuracy-aware displacement as well as reported speed. Rejected
    observations never create distance, stationary minutes, or individual stops.
    """
    ...
    for row in ordered:
        try: lat=float(row.latitude);lon=float(row.longitude);acc=max(3.0,float(row.accuracy_m or 0));speed=max(0.0,float(row.speed_kmh or 0))
        except (TypeError,ValueError):reject(row,'INVALID_PAYLOAD');continue
        if not(-90<=lat<=90 and -180<=lon<=180) or (abs(lat)<.000001 and abs(lon)<.000001):reject(row,'INVALID_COORDINATES');continue
        if acc>150:reject(row,'POOR_ACCURACY');continue
        valid.append({'latitude':lat,'longitude':lon,'accuracy':acc,'speed':speed,'timestamp':aware(row.sampled_at).isoformat()})
    segments=[];current=[];stationary=[];stops=[];moving_seconds=0.0
    for point in valid:
        if not current:
            current=[point];stationary=[];continue
        previous=current[-1];seconds=(datetime.fromisoformat(point['timestamp'])-datetime.fromisoformat(previous['timestamp'])).total_seconds()
        if seconds<=0 or seconds>300:
            if len(current)>=2:segments.append(current)
            current=[point];stationary=[];continue
        metres=_distance_dict(previous,point)*1000
        implied_kmh=(metres/seconds)*3.6 if seconds else 0
        movement_threshold=max(8.0,min(35.0,(previous['accuracy']+point['accuracy'])*0.45))
        # Never promote a long GPS jump into a route merely because its calculated
        # speed is below a broad vehicle ceiling. Phone fixes with zero/near-zero
        # reported speed may prove walking movement only at a plausible walking
        # rate. Vehicle movement must agree reasonably with the reported speed.
        impossible_jump=metres>max(120.0,45.0*seconds) or implied_kmh>160
        if impossible_jump:
            reject(type('RoutePoint',(),{'sampled_at':datetime.fromisoformat(point['timestamp']),'latitude':point['latitude'],'longitude':point['longitude'],'accuracy_m':point['accuracy'],'speed_kmh':point['speed']})(),'IMPOSSIBLE_JUMP')
            if len(current)>=2:segments.append(current)
            current=[point];stationary=[];continue
        if point['speed']<1.0:
            moving=metres>=movement_threshold and 1.0<=implied_kmh<=20.0 and seconds<=60
        else:
            reported_ceiling=max(20.0,point['speed']*2.5+15.0)
            moving=metres>=movement_threshold and implied_kmh<=reported_ceiling and seconds<=90
        if moving:
            if stationary:
                duration=(datetime.fromisoformat(stationary[-1]['timestamp'])-datetime.fromisoformat(stationary[0]['timestamp'])).total_seconds()
                if duration>=120 and segments:
                    stops.append({'started_at':stationary[0]['timestamp'],'ended_at':stationary[-1]['timestamp'],'minutes':round(duration/60),'latitude':stationary[-1]['latitude'],'longitude':stationary[-1]['longitude'],'accuracy':stationary[-1]['accuracy'],'reason':'CONFIRMED_STOP'})
            stationary=[];current.append(point);moving_seconds+=seconds
        else:
            stationary.append(point)
            # Keep one stationary anchor but do not create fake route distance.
            if len(current)>=2:segments.append(current)
            current=[point]
    if len(current)>=2:segments.append(current)
    accepted=[point for segment in segments for point in segment]
    # De-duplicate shared segment boundary points.
    unique=[];seen=set()
    for point in accepted:
        key=point['timestamp']
        if key not in seen:unique.append(point);seen.add(key)
    accepted=unique
    ...
    return {'points':accepted,'last':last,'accepted':accepted,'rejected':rejected,'segments':segments,'journeys':journeys,'stops':stops,'total_km':round(distance,2),'distance_km':round(distance,2),'max_speed':round(maximum),'maximum_speed':round(maximum),'moving_minutes':round(moving_seconds/60),'movement_minutes':round(moving_seconds/60),'stopped_minutes':stopped_minutes,'stationary_minutes':stopped_minutes,'rejection_counts':rejection_counts,'confidence':confidence,'state':state,'raw_count':len(ordered),'movement_count':len(accepted),'drift_count':max(0,len(valid)-len(accepted)),'rejected_count':len(rejected),'low_quality_count':sum(1 for x in valid if x['accuracy']>50),'raw_speed':valid[-1]['speed'] if valid else None}
...
def tracking_safety_save(asset_id):
    ...
    try:
        speed=max(0.0,min(250.0,float(request.form.get('speed_limit_kmh') or 0)))
