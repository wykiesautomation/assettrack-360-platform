...
def device_connectivity_state(device, reference_time=None):
    """Single dashboard connectivity truth based only on real device contact."""
...
def connect_device():
    ...
    requested_asset=Asset.query.filter_by(id=requested_asset_id,customer_id=customer_id).first() if requested_asset_id else None
    preselected_asset=next((a for a in assets if a.id==requested_asset_id),None)
    requested_asset_block_reason=None
    if requested_asset and requested_asset.id in active_asset_ids:
        linked=Device.query.filter_by(customer_id=customer_id,asset_id=requested_asset.id,active=True).order_by(Device.id).first()
        requested_asset_block_reason=f'{requested_asset.name} already has active device {linked.device_uid}.' if linked else f'{requested_asset.name} already has an active device.'
    if request.method=='POST':
        ...
        if asset_mode=='new':
            ...
            if kind=='ANDROID_PHONE' and asset.asset_type!='TRACKER':
                # A specifically selected, unassigned GENERIC asset is a valid
                # mobile-onboarding record. Convert that same asset safely so
                # the entered name and Asset ID are retained. Never create a
                # duplicate and never convert tank/vibration process assets.
                if asset.asset_type=='GENERIC' and asset.status=='UNASSIGNED' and asset.id==request.form.get('requested_asset_id',type=int):
                    asset.asset_type='TRACKER'
                    metadata=dict(asset.metadata_json or {})
                    metadata.update({'onboarding_source':'ANDROID_EXISTING_ASSET','converted_from_asset_type':'GENERIC','mobile_tracker':True})
                    asset.metadata_json=metadata
                else:
                    flash('Android Phone requires a Tracker asset. Only the selected unassigned Generic asset can be converted safely.','error');return redirect(url_for('main.connect_device',asset_id=asset.id))
        ...
        return redirect(url_for('main.connect_device_waiting'))
    return render_template('connect_device.html',assets=assets,sites=sites,device_profiles=public_profiles(),has_sites=bool(sites),has_assets=bool(assets),preselected_asset=preselected_asset,requested_asset=requested_asset,requested_asset_block_reason=requested_asset_block_reason)
...
def mobile_tracker_register():
    ...
    client_version=str(data.get('client_version') or 'mobile-web-2.0')[:40];platform=re.sub(r'[^A-Z0-9_-]','',str(data.get('platform') or 'WEB').upper())[:20] or 'WEB';device_type='ANDROID_MOBILE_TRACKER' if platform=='ANDROID' else 'IOS_MOBILE_TRACKER' if platform=='IOS' else 'MOBILE_WEB_TRACKER';device=Device(customer_id=reg.customer_id,asset_id=reg.asset_id,device_uid=reg.device_uid,device_type=device_type,api_token=token,active=True,firmware=client_version,capabilities=['GPS','PHONE_BATTERY','USER_CONSENT_REQUIRED','PLATFORM:'+platform,'PROFILE:AT360_MOBILE_TRACKER'])
