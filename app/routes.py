...
def build_validated_trips(rows,speed_limit=60.0):
    ...
    for row in sorted(rows,key=lambda x:(aware(x.sampled_at),getattr(x,'id',0) or 0)):
        try:
            point={'latitude':float(row.latitude),'longitude':float(row.longitude),'accuracy':max(3.0,float(row.accuracy_m or 0)),'reported_speed':max(0.0,float(row.speed_kmh or 0)),'timestamp':aware(row.sampled_at).isoformat()}
        except (TypeError,ValueError):
    ...
    for point in raw:
        ...
        if continuous and moving:current.append(point)
...
@login_required
def asset_trips(asset_id):
    asset=Asset.query.filter_by(id=asset_id,customer_id=tenant_id()).first_or_404();now=utcnow();sast=timezone(timedelta(hours=2));local_now=now.astimezone(sast)
    try:start=datetime.fromisoformat(request.args.get('from')).replace(tzinfo=sast).astimezone(timezone.utc) if request.args.get('from') else local_now.replace(hour=0,minute=0,second=0,microsecond=0).astimezone(timezone.utc)
    except ValueError:start=local_now.replace(hour=0,minute=0,second=0,microsecond=0).astimezone(timezone.utc)
    try:end=(datetime.fromisoformat(request.args.get('to')).replace(hour=23,minute=59,second=59,tzinfo=sast).astimezone(timezone.utc) if request.args.get('to') else now)
