...
def asset_view(asset_id):
    ...
    # A TANK asset already renders its primary level signal in Live Tank Inventory.
    # Do not render the same signal as a second tank graphic in Process Monitoring.
    # Other independent assigned channels remain visible and no telemetry is deleted.
    if asset.asset_type=='TANK' and primary_visual_signal:
        universal_signal_cards=[card for card in universal_signal_cards if card['signal'].id!=primary_visual_signal.id]
