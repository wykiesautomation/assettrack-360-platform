...
def delete_device(device_id):
    """Permanently remove one disabled device, never its asset or site."""
    ...
    if confirm_uid!=expected_uid or confirm_word!='DELETE':
        flash(f'Type the exact Device UID {expected_uid} and DELETE. The site and asset are not deleted.','error')
        return redirect(request.referrer or url_for('main.devices'))
    ...
    try:
        # Revoke access before any relationship cleanup. A deleted board can never
        # authenticate again with the previous token.
        record.active=False
        record.api_token='REVOKED-'+secrets.token_urlsafe(32)
        db.session.flush()

        # Clean every deployed FK that points at device.id. Nullable historical
        # references are detached; device-owned operational rows are removed.
        # Asset, Site, Reading, Location and Alarm rows are deliberately untouched.
            ...
            for fk in inspector.get_foreign_keys(table_name):
                ...
                if columns.get(column_name,{}).get('nullable',True):
                    db.session.execute(text(
                        f'UPDATE {table_sql} SET {column_sql}=NULL WHERE {column_sql}=:device_id'
                    ),{'device_id':record.id})
                else:
                    db.session.execute(text(
                        f'DELETE FROM {table_sql} WHERE {column_sql}=:device_id'
                    ),{'device_id':record.id})
        ...
        # Regression guards: these parent records must still exist.
        if asset_id and not db.session.get(Asset,asset_id):
            raise RuntimeError('asset_retention_guard_failed')
        if site_id and not db.session.get(Site,site_id):
            raise RuntimeError('site_retention_guard_failed')

        db.session.commit()
        flash(f'Device {expected_uid} permanently deleted. Asset and site retained.','ok')
    except Exception as exc:
        db.session.rollback()
        current_app.logger.exception('Permanent device delete failed device_id=%s customer_id=%s',device_id,customer_id)
        flash(f'Device deletion failed safely: {type(exc).__name__}. No asset or site was deleted.','error')
