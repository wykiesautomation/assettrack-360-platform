...
def analyse_tracking_points(rows):
    ...
    for row in ordered:
        ...
        valid.append({'row':row,'latitude':lat,'longitude':lon,'accuracy':acc,'speed':speed,'timestamp':aware(row.sampled_at).isoformat()})
    ...
    for point in valid:
        ...
        movement_threshold=max(8.0,min(40.0,(previous['accuracy']+point['accuracy'])*0.45))
        moving=(metres>=movement_threshold and implied_kmh<=180) or (point['speed']>=3 and metres>=8 and implied_kmh<=180)
        if moving:
            if stationary:
