(() => {
  ...
  // Phone camera QR links may open inside a restricted in-app browser where
  // localStorage throws SecurityError. Registration must not succeed on the
  // server and then appear unregistered merely because browser storage failed.
  const volatileStore=new Map();
  function storageGet(key){
    for(const store of [localStorage,sessionStorage]){try{const value=store.getItem(key);if(value!==null)return value;}catch{}}
    return volatileStore.has(key)?volatileStore.get(key):null;
  }
  function storageSet(key,value){
    let persisted=false;
    for(const store of [localStorage,sessionStorage]){try{store.setItem(key,value);persisted=true;}catch{}}
    volatileStore.set(key,value);
    return persisted;
  }
  function storageRemove(key){
    for(const store of [localStorage,sessionStorage]){try{store.removeItem(key);}catch{}}
    volatileStore.delete(key);
  }
  function read(key,fallback=null){try{const x=storageGet(key);return x?JSON.parse(x):fallback;}catch{return fallback;}}
  function write(key,value){return storageSet(key,JSON.stringify(value));}
  ...
  function trackingWanted(){return storageGet(TRACKING_KEY)==='true';}
  function setTrackingWanted(v){storageSet(TRACKING_KEY,v?'true':'false');}
  ...
  async function uploadPayload(payload,queueOnFailure=true){
    ...
    try{
      ...
      if(!r.ok){
        if(r.status===401||r.status===403){setTrackingWanted(false);stopWatch();log('Upload authorization failed. Device identity retained. Reopen this page in Chrome and retry Start Tracking.');return false;}
  ...
  async function registerPhone(){const code=(el('code')?.value||'').trim().toUpperCase();if(!el('consentCheck')?.checked){log('Accept the privacy notice.');return;}if(code.length<8){log('Enter the full registration code.');return;}setRegisterBusy(true);try{const r=await fetch(API.register,{method:'POST',cache:'no-store',credentials:'same-origin',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({code,consent:true,policy_version:'2026.1',platform:platform(),client_version:'mobile-web-2.0'})});const d=await safeJson(r);if(!r.ok)throw Error(d.error||`HTTP ${r.status}`);const registered={device_uid:d.device_uid,token:d.device_token,asset_name:d.asset_name||'Mobile Tracker',sequence:0,last_success_at:0};const persisted=saveState(registered);saveQueue([]);setTrackingWanted(true);showTracker(loadState()||registered);await loadConfig();log(`Phone registered as ${d.device_uid}. Starting tracking now.`);if(!persisted)log('Camera browser storage is restricted. Keep this page open or use Open in Chrome so registration remains saved.');beginWatch(false);}catch(e){log(`Registration failed: ${e.message}`);}finally{setRegisterBusy(false);}}
  async function withdrawConsent(){stopTracking(false);if(await sendEvent('CONSENT_WITHDRAWN')){storageRemove(STORAGE_KEY);storageRemove(QUEUE_KEY);showRegistration();}}
  async function unregisterPhone(){if(!confirm('Unregister this phone and revoke its token?'))return;stopTracking(false);await sendEvent('UNREGISTERED');storageRemove(STORAGE_KEY);storageRemove(QUEUE_KEY);showRegistration();}
  async function requestDataDeletion(){if(await sendEvent('DATA_DELETION_REQUESTED'))log('Deletion request submitted.');}
  function clearLocalTrackerData(){stopTracking(false);storageRemove(STORAGE_KEY);storageRemove(QUEUE_KEY);showRegistration();log('Local tracker data cleared.');}
