(() => {
  ...
  async function uploadPayload(payload,queueOnFailure=true){
    ...
    try{
      ...
      if(!r.ok){
        if(r.status===401||r.status===403){setTrackingWanted(false);stopWatch();log(`Location authorization failed: ${data.error||'HTTP '+r.status}. Registration retained.`);return false;}
  ...
  async function sendEvent(event){const s=loadState();if(!s)return false;const endpoint=event==='TRACKING_STARTED'?API.start:event==='TRACKING_STOPPED'?API.stop:API.event;try{const r=await fetch(endpoint,{method:'POST',cache:'no-store',headers:authHeaders(s),body:JSON.stringify({event,client_version:'mobile-web-2.1'})});const d=await safeJson(r);if(!r.ok)log(`${event} rejected: ${d.error||'HTTP '+r.status}`);return r.ok;}catch(e){log(`${event} delayed: ${e.message}`);return false;}}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    el('registerBtn')?.addEventListener('click',registerPhone);
    const linkedCode=(el('code')?.value||'').trim();
    const saved=loadState();
    if(linkedCode){
      // A newly scanned one-time QR is an explicit onboarding request. Never
      // skip consent or reuse an older phone identity merely because this
      // browser already contains a previous tracker token.
      stopWatch();
      setTrackingWanted(false);
      showRegistration();
      if(el('consentCheck'))el('consentCheck').checked=false;
      log('Secure one-time QR loaded. Review consent, then press Register Phone. Registration has not happened yet.');
      el('consentCheck')?.focus();
    }else if(saved?.device_uid&&saved?.token){
      showTracker(saved);
      loadConfig().then(()=>{if(trackingWanted())beginWatch(true);});
    }else{
      showRegistration();
    }
    updateNetwork();updateQueue();healthId=setInterval(refreshStatus,15000);log('Tracker controls loaded.');
  });
