(() => {
  ...
  function orientationHandler(e){const roll=Number(e.gamma||0),pitch=Number(e.beta||0),now=Date.now();lastOrientation={roll,pitch};if(el('orientationValue'))el('orientationValue').textContent=`${roll.toFixed(1)}° / ${pitch.toFixed(1)}°`;if(calibrating){calibrationSamples.push({roll_deg:roll,pitch_deg:pitch});const target=60;if(el('calibrationProgress'))el('calibrationProgress').textContent=`${Math.min(calibrationSamples.length,target)}/${target}`;if(calibrationSamples.length>=target)finishMotionCalibration();}if(Math.abs(roll)>=65||Math.abs(pitch)>=110){if(!tiltStartedAt)tiltStartedAt=now;if(Date.now()-tiltStartedAt>=10000&&now-lastTiltEventAt>900000){lastTiltEventAt=now;sendCandidate(candidatePayload('ABNORMAL_TILT',{confidence:.72,orientation_duration_seconds:(now-tiltStartedAt)/1000}));}}else tiltStartedAt=0;}
