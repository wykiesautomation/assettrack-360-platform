...
def test_shared_connectivity_engine():
    assert "def device_connectivity_state" in R
    for x in ("return 'ONLINE'", "return 'DELAYED'", "return 'OFFLINE'", "asset_connectivity=asset_connectivity"):
        assert x in R
    assert "age_seconds<=90" in R
    assert "age_seconds<=300" in R
